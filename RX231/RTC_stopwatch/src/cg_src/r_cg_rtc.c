...
void R_RTC_Create(void)
{
    ...
    RTC.RCR1.BYTE = _00_RTC_ALARM_INT_DISABLE | _00_RTC_CARRY_INT_DISABLE | _04_RTC_PERIOD_INT_ENABLE | 
                    _80_RTC_PERIODIC_INT_PERIOD_1_64;
    ...
    RTC.RTCCR1.BYTE = _02_RTC_TIME_CAPTURE_FALL | _00_RTC_TIME_CAPTURE_NOISE_FILTER_OFF | 
                      _80_RTC_TIME_CAPTURE_EVENT_ENABLE;
...
* Description  : This function restarts RTC counter in calendar mode.
...
/***********************************************************************************************************************
* Function Name: R_RTC_Get_CalendarTimeCaptureValue1
* Description  : This function read the results of time capture registers in calendar mode.
* Arguments    : counter_read_val -
*                    counter read value
* Return Value : None
***********************************************************************************************************************/
void R_RTC_Get_CalendarTimeCaptureValue1(rtc_calendarcounter_value_t * const counter_read_val)
{
    uint8_t tcen_copy;
    uint8_t tcct_copy;

    /* Time capture event 1 has been detected */
    if (1U == RTC.RTCCR1.BIT.TCST)
    {
        /* Store TCCT, TCEN before setting */
        tcen_copy = RTC.RTCCR1.BIT.TCEN;
        tcct_copy = RTC.RTCCR1.BIT.TCCT;

        /* Disable event 1 detection */
        RTC.RTCCR1.BIT.TCCT = 0U;
        while (0U != RTC.RTCCR1.BIT.TCCT);

        /* Read calendar time capture value */
        counter_read_val->rseccnt = RTC.RSECCP1.BYTE;
        counter_read_val->rmincnt = RTC.RMINCP1.BYTE;
        counter_read_val->rhrcnt = RTC.RHRCP1.BYTE;
        counter_read_val->rdaycnt = RTC.RDAYCP1.BYTE;
        counter_read_val->rmoncnt = RTC.RMONCP1.BYTE;

        /* Clear TCST */
        RTC.RTCCR1.BIT.TCST = 0U;
        while (0U != RTC.RTCCR1.BIT.TCST);

        /* Restore TCCT setting */
        RTC.RTCCR1.BIT.TCCT = tcct_copy;
        while (tcct_copy != RTC.RTCCR1.BIT.TCCT);

        /* Restore TCEN setting */
        RTC.RTCCR1.BIT.TCEN = tcen_copy;
        while (tcen_copy != RTC.RTCCR1.BIT.TCEN);
    }
    else
    {
        /* Read calendar time capture value */
        counter_read_val->rseccnt = 0U;
        counter_read_val->rmincnt = 0U;
        counter_read_val->rhrcnt = 0U;
        counter_read_val->rdaycnt = 0U;
        counter_read_val->rmoncnt = 0U;
    }
}
